...
let standardPlanId = "standard-monthly";
let basicPlanId = "basic-monthly";

let weekInSeconds = 604800;

let halfWeekInSeconds = weekInSeconds/2;
    ...
    jobIds.forEach(jobId => {
      ...
      db.ref('/websites/' + job.siteKey).once('value').then(
        websiteRef => {
          ...
          if(website.planId == "standard-monthly" && website.lastCrawlTime + halfWeekInSeconds > Date.now()){
            return;
          }

          if(website.planId == "basic-monthly" && website.lastCrawlTime + weekInSeconds > Date.now()){
            return;
          }
