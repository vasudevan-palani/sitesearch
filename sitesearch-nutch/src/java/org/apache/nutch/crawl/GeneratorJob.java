...
public class GeneratorJob extends NutchTool implements Tool {
  ...
  public static void main(String args[]) throws Exception {
    ...
    if(!crawlId.equalsIgnoreCase("")){
      LOG.info("Loading with crawlId,"+crawlId);
      conf = NutchConfiguration.create(crawlId);
    }
    else {
      LOG.info("Loading without crawlId,"+crawlId);
      conf = NutchConfiguration.create();
    }
    int res = ToolRunner.run(conf, new GeneratorJob(),
        args);
    System.exit(res);
  }
